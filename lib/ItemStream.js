...
ItemStream.prototype.getTradeItemsFromStream = function (is, cb) {
  var tradeItems = []
  this.getEachTradeItem(is, function (err, item) {
...
ItemStream.prototype.getEachTradeItem= function (is, cb) {
  ...
  msg_info_extract.on('msg_info', function (err, msg_info) {
    ...
    msg_info_success = msg_info.recipient
  ...
  msg_info_extract.on('end', function () {
    console.log('msg_info_extract -> end with info: ' + JSON.stringify(msg_info_success))
    if (!msg_info_success) cb (new Error('required message header info not found'))
  })
