...
module.exports = function MessageInfo(xml, config) {
  ...
  */
  ...
  var root_prefix = root_parts && root_parts[0]
  ...
  var header_prefix = ''
  if (root_local_name == 'StandardBusinessDocument') { //return this // all done if older 2.8 schema
  ...
  if (msg_info.msg_type == 'basicPartyRegistration') {
    $('basicPartyRegistrationIdentification').each(function () {
      var gln = $('informationProviderOfParty gln', this.parent).text() 
