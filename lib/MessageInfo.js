...
var MessageInfo = module.exports = function (msg_xml, config) {
  ...
  if (msg_info.msg_type == 'catalogueItemConfirmation') {
    ...
    msg_info.status = $('catalogueItemConfirmationState').attr('state') // 2.8
      || $('catalogueItemConfirmationStateCode').text()                 // 3.1
    console.log('cic state code: ' + msg_info.status)

    if (msg_info.status == 'REVIEW') {
      // capture reason for review
      var review_msg = $('catalogueItemConfirmationStatus').text() // includes status code and description
      if (review_msg) {
    ...
    msg_info.recipient_dp = $('catalogueItemConfirmationState > recipientDataPool').text()
