...
var MessageInfo = module.exports = function (msg_xml, config) {
  ...
  if (root_parts && root_parts.length == 2) {
    ...
    log('msg ns root_prefix: ' + root_prefix)
  }
  ...
  if (!msg.msg_type) {
    ...
    log('header prefix: ' + header_prefix)
    log('root local name: ' + root_local_name)

    if (root_local_name == 'tradeItems') { // custom MDS flat list of tradeItem XML
  ...
  log('WARNING: msg type not recognized: ' + msg.msg_type)
  return msg
}
