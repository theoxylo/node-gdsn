...
var MessageInfo = module.exports = function (xml, config) {

  config = config || {}

  var msg_info = this // rename this for uniform use with callbacks
  msg_info.reset() // resetting all properties to empty strings and arrays
  ...
  msg_info.xml = xml
  msg_info.xml_length = xml.length
  ...
  if (msg_info.msg_type == 'gS1Response') {
    ...
    if (exception) {
      ...
      if (msg_info.msg_type == 'partyRegistrationResponseMessage') {
        try {
          msg_info.provider = $('partyReference').first().text() // the party gln
        }
        catch (e) {}
      }
    }
  ...
  if (msg_info.msg_type == 'basicPartyRegistration') {
    ...
    $('basicPartyRegistrationIdentification').each(function () {
      ...
      var trx_id = $('transactionIdentification > entityIdentification', this.parent.parent.parent).first().text()
      var cmd_id = $('documentCommandIdentification > entityIdentification', this.parent.parent).first().text()
  ...
  if (msg_info.msg_type == 'registryPartyDataDump') {
    ...
    console.log('RPDD...........................')
      // we may need these additional parent IDs to report errors
      var trx_id = $('transactionIdentification > entityIdentification', this.parent.parent.parent).first().text()
      var cmd_id = $('documentCommandIdentification > entityIdentification', this.parent.parent).first().text()
      var party = new PartyInfo($(this.parent).toString(), msg_info.msg_id, trx_id, cmd_id)
      msg_info.party.push(party)
  ...
  if (msg_info.msg_type == 'catalogueItemNotification') {
    ...
    $('tradeItem').each(function () {
      ...
      console.log('TESTING parents: ' + $(this).parentsUntil())
      console.log('TESTING parents trx id: ' + $(this).parents('transactionIdentification > entityIdentification', this))

      var trx_id = $('transactionIdentification > entityIdentification', this.parent.parent.parent).first().text()
      var cmd_id = $('documentCommandIdentification > entityIdentification', this.parent.parent).first().text()
      var item = new ItemInfo($(this).toString(), msg_info.msg_id, trx_id, cmd_id)
      
      msg_info.item.push(item)

      msg_info.gtins.push(item.gtin) // legacy
    })
...
MessageInfo.prototype.reset = function () {
