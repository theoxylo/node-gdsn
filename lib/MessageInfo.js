...
var MessageInfo = function () {
  if (!(this instanceof MessageInfo)) return new MessageInfo()
  console.log('Creating new instance of MessageInfo')
}

// handles fully formed xml
MessageInfo.prototype.populate = function (xml) {
  console.log('Populating instance of MessageInfo')
  ...
  var clean_xml = MessageInfo.config.gdsn.clean_xml(msg.xml)
  ...
  if (msg.msg_type == 'GDSNResponse') {
    ...
    if (exception) {
      ...
      //if (!msg.status) msg.status = $('partyRegistrationResponse').first().attr('responseStatus')
      //if (!msg.status) msg.status = $('catalogueItemRegistrationResponse').first().attr('responseStatus')
      msg.status = 'ACCEPTED'
    }
  }
    ...
    try {
      ...
      if (!msg.source_dp) console.log('sourceDataPool element not set')
    }
    catch (err) { 
      console.log('sourceDataPool element not found, err: ' + err)
    }
