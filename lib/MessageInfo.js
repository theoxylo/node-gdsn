...
var MessageInfo = module.exports = function (msg_xml, config) {
  ...
  // response/exception messages including PRR and CIRR
  if (msg_info.msg_type == 'gS1Response'
    ...
    if (exception) {
      ...
      if (msg_info.msg_type == 'catalogueItemRegistrationResponse') {
        try {
          ...
          msg_info.tm       = $('catalogueItemReference > targetMarket > targetMarketCountryCode').text()
          msg_info.tm_sub   = $('catalogueItemReference > targetMarket > targetMarketSubdivisionCode').text() || 'na'
        }
        catch (e) {}
      }
  ...
  //console.log('message status: ' + msg_info.status)
  //console.log('message gpc: ' + msg_info.gpc)
  ...
  if (msg_info.msg_type == 'registryCatalogueItem') {
    $('registryCatalogueItemIdentification').each(function () {
      var gtin = $('catalogueItemReference > gtin', this.parent).text()
      msg_info.gtin = gtin
      msg_info.gtins.push(gtin)
      msg_info.provider = $('catalogueItemReference > dataSource', this.parent).text()
      msg_info.tm       = $('catalogueItemReference > targetMarket > targetMarketCountryCode', this.parent).text()
      msg_info.tm_sub   = $('catalogueItemReference > targetMarket > targetMarketSubdivisionCode', this.parent).text() || 'na'
    })
  ...
  if (msg_info.msg_type == 'catalogueItemSubscription') {
    ...
    .first()
    .each(function () { // only support one subscription per message for now
        var trx_id = $('transactionIdentification > entityIdentification', this.parent.parent.parent).text()
        var cmd_id = $('documentCommandIdentification > entityIdentification', this.parent.parent).text()
        ...
        //msg_info.source_dp = sub.recipient_dp // source of items or source of subs? probably items which would be local dp
  ...
  if (msg_info.msg_type == 'catalogueItemPublication') {
    $('catalogueItemPublicationIdentification').first().each(function () {
    ...
    msg_info.source_dp = msg_info.receiver
    return msg_info
  }
  ...
  if (msg_info.msg_type == 'catalogueItemConfirmation') {
    msg_info.status = $('catalogueItemConfirmationState').attr('state')
