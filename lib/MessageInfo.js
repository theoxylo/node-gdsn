...
var MessageInfo = module.exports = function (msg_xml, config) {
  ...
  if (msg_info.msg_type == 'registryCatalogueItem') {
    $('registryCatalogueItemIdentification').each(function () {
      ...
      msg_info.tm_sub   = $('catalogueItemReference > countrySubDivisionISOCode', this.parent).text() || 'na'
    })
  ...
  if (msg_info.msg_type == 'catalogueItemSubscription') {
    $('catalogueItemSubscriptionIdentification').each(function () {
      ...
      if (!msg_info.recipient) {
        // repeat single item details at message level for easy display
  ...
  if (msg_info.msg_type == 'catalogueItemPublication') {
    $('catalogueItemPublicationIdentification').each(function () {
      ...
      if (!msg_info.recipient) {
        // populate first pub info at msg level for easy reference
  ...
  if (msg_info.msg_type == 'catalogueItemNotification') {
    ...
    console.log('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CIN msg_info: ' + JSON.stringify(msg_info))
    ...
    if (!msg_info.tm && msg_info.item[0]) { // copy some values from first item to message level
      // these should be the same for all items in a hierarchy
      msg_info.tm     = msg_info.item[0].tm
      msg_info.tm_sub = msg_info.item[0].tm_sub
      msg_info.gpc    = msg_info.item[0].gpc
    }
