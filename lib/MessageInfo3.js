...
module.exports = function MessageInfo3(xml, config) {
  ...
  msg_info.msg_id = $(header_prefix + '\\:InstanceIdentifier').text()
  ...
  if (msg_info.msg_type == 'partyRegistrationResponseMessage') {
    ...
    msg_info.status = party_res.find('responseStatusCode').text()
    console.log('status code: ' + msg_info.status)
    return msg_info
  }

  // response/exception messages
  else if (msg_info.msg_type == 'gS1ResponseMessage') {
