...
var PublicationInfo = module.exports = function (pub_xml, msg_id, trx_id, cmd_id) {
  ...
  if (!this.tm) this.tm = $('catalogueItemReference > targetMarket > targetMarketCountryCode').text() // 3.1

  this.tm_sub                  = $('catalogueItemReference > countrySubDivisionISOCode').text()    // 2.8
  if (!this.tm_sub) this.tm_sub = $('catalogueItemReference > targetMarket > targetMarketSubdivisionCode').text() // 3.1
