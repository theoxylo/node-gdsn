...
var TradeItemInfo = module.exports = function (ti_xml, msg_id, trx_id, cmd_id, doc_id) {
  ...
  item.provider  = $('tradeItem > informationProviderOfTradeItem > gln').text()                     // 3.1
                ...
                || $('tradeItem > extension > mdsExtension > vendorId').text()                        // custom MDS extension
  
  item.recipient = $('tradeItem>extension>mdsExtension>buyerId').text() // custom MDS extension
                || item.provider // default recipient is the provider, making this a "draft item", recipient can be changed to DP or subscriber later

  item.gtin      = $('tradeItem>gtin')                          .text()            // 3.1
                || $('tradeItem>tradeItemIdentification>gtin').text()            // 2.8
                || $('tradeItem>extension>mdsExtension>itemOwnerProductCode').text() // custom MDS extension
  ...
  item.tm_sub    = $('targetMarket>targetMarketSubdivisionCode').text() // 3.1
                ...
                || $('tradeItem>extension>mdsExtension>portalChar').text() // custom MDS extension
                || 'na' // default
