...
var TradeItemInfo = module.exports = function (ti_xml, msg_id, trx_id, cmd_id, doc_id) {
  ...
  item.tm        = $('tradeItem > targetMarket > targetMarketCountryCode').first().text() // 3.1 unbounded
  ...
  item.tm_sub    = $('tradeItem > targetMarket > targetMarketSubdivisionCode').first().text() // 3.1 unbounded
  ...
  // mds attributes:
  item.vendorId             = $('extension>mdsOmsExtension>vendorId').text()              // custom MDS extension
  item.buyerId              = $('extension>mdsOmsExtension>buyerId').text()               // custom MDS extension
  item.itemOwnerProductCode = $('extension>mdsOmsExtension>itemOwnerProductCode').text()  // custom MDS extension
  item.portalChar           = $('extension>mdsOmsExtension>portalChar').text()            // custom MDS extension
  if (item.gtin) {
    item.gdsn = true
  }
  else {
