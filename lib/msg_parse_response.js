  ...
  if (msg.msg_type == 'gS1Response'                       // 3.1, includes gS1Exception
   ...
   || msg.msg_type == 'catalogueItemRegistrationResponse' // 3.1, CIRR response to RCI
    ...
    if (exception) {
      ...
      // CIRR from GR, response to RCI
      else if (msg.msg_type == 'GDSNResponse') { // could be 2.8 CIRR (GR response to RCI)
        msg.gtin     = $('catalogueItemReference > gtin').text()
        msg.provider = $('catalogueItemReference > dataSource').text()
        msg.tm       = $('catalogueItemReference > targetMarket > targetMarketCountryCode > countryISOCode').text()
        msg.tm_sub   = $('catalogueItemReference > targetMarket > targetMarketSubdivisionCode > countrySubDivisionISOCode').text() || 'na'
      }
    }
