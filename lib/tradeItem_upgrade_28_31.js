...
module.exports = function (cheerio, gdsn) {
  ...
  function create_variants ($28, $31) {
    ...
    var $tii = $31('tradeItemInformation').first().remove()
    $28('foodAndBeverageInformation').each(function () {
      log('found fb info element')
      var $new_tii = $tii.clone() // already populated with all common information
      ...
      $28('productionVariantDescription', this).each(function () {
        ...
        if (lang && text) {
          $31('productionVariantDescription', $new_tii).after($31('<productionVariantDescription/>').attr('languageCode', lang).text(text))
        }
      })
      $31('productionVariantDescription', $new_tii).first().remove()

      var pved = $28('productionVariantEffectiveDate', this).text()
      if (pved) $31('productionVariantEffectiveDateTime', $new_tii).text(pved + 'T01:01:01.009Z')
      else $31('productionVariantEffectiveDateTime', $new_tii).remove()

      var allergens = $31('allergenRelatedInformation', $new_tii)
      $28('foodAndBeverageAllergyRelatedInformation', this).each(function () {
        var spec_agency = ''
        var spec_name   = ''
        ...
        $28('allergenStatement > description', this).each(function () {
          ...
          if (lang && text) $31('allergenStatement', allergens).after($31('<allergenStatement/>').attr('languageCode', lang).text(text))
        })
        $31('allergenStatement', allergens).first().remove()
      })

      $31('tradeItemSynchronisationDates').before($new_tii)
