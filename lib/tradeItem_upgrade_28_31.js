...
module.exports = function (cheerio, gdsn) {
  ...
  return function (ti_28_xml) {
    ...
    populate_sales_info($28, $31, $extension)
  ...
  function create_first_or_non_variant($28, $31) {
    log('create_first_or_non_variant')

    var $tii = $31('tradeItem > tradeItemInformation').first() // populate first target info only
    var $extension = $31('<extension/>')
    $tii.append($extension)

    $28('foodAndBeverageInformation').first().each(function () { // look at first food bev info only
      ...
      if (pved) $tii.append($31('<productionVariantEffectiveDateTime/>').text(pved + 'T01:01:01.009Z'))
  ...
  function create_additional_variants($28, $31) {
    ...
    $28('foodAndBeverageInformation').each(function () {
      ...
      $28('productionVariantDescription', this).each(function () {
        ...
        if (lang && text) {
          if (!$tii) {
              $tii = $31('<tradeItemInformation/>')
              $31('tradeItem > tradeItemSynchronisationDates').before($tii)
          }
          log('found variant: ' + text)
          $tii.append($31('<productionVariantDescription/>').attr('languageCode', lang).text(text))
        }
      ...
      else  return // no additional variants to process
...
*/
  ...
  function populate_measurements($28, $31, $extension) {
    ...
    $28('tradeItemMeasurements > netContent > measurementValue').each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
    ...
    $28('tradeItemMeasurements > drainedWeight > measurementValue').each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
    ...
    $28('tradeItemMeasurements > grossWeight > measurementValue').each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
...
/*
      <sales_information:salesInformationModule xmlns:sales_information="urn:gs1:gdsn:sales_information:xsd:3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:gs1:gdsn:sales_information:xsd:3 http://www.gdsregistry.org/3.1/schemas/gs1/gdsn/SalesInformationModule.xsd">
        <salesInformation>
...
*/
  function populate_sales_info($28, $31, $extension) {
