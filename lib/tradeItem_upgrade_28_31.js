...
module.exports = function (cheerio, gdsn) {
  ...
  return function (ti_28_xml) {
    ...
    var lcdt = $28('tradeItemDateInformation > lastChangeDateTime').first().text() || now // required in 3.1
    $31('tradeItemSynchronisationDates > lastChangeDateTime').text(lcdt)

    var canceled_date = $28('tradeItemDateInformation > canceledDate').first().text()
    if (canceled_date) {
    ...
    var cvdt = $28('tradeItemDateInformation > communityVisibilityDateTime').first().text()
    if (cvdt) {
    ...
    var discontinued_date = $28('tradeItemDateInformation > discontinuedDate').first().text()
    ...
    var effective_date = $28('tradeItemDateInformation > effectiveDate').first().text()
    if (effective_date) {
    ...
    var publication_date = $28('tradeItemDateInformation > publicationDate').first().text()
    if (publication_date) {
