...
module.exports = function (cheerio, gdsn) {
  ...
  return function (ti_28_xml) {
    ...
    $31('informationProviderOfTradeItem > partyName'   ).text($28('informationProviderOfTradeItem > nameOfInformationProvider').first().text() || 'na')

    $28('informationProviderOfTradeItem > informationProvider > additionalPartyIdentification').each(function () {
      var id   = $28('additionalPartyIdentificationValue', this).text()
      var type = $28('additionalPartyIdentificationType', this).text()
      $31('informationProviderOfTradeItem > gln').before('<additionalPartyIdentification additionalPartyIdentificationTypeCode="' + type + '">' + id + '</additionalPartyIdentification>')
    })
    ...
    $28('tradeItemDescriptionInformation').each(function () { // should be only 1
      $31('brandName').text($28('brandName').text())
      
      $28('descriptionShort > description', this).each(function () {
        var lang = $28('languageISOCode', this).text()
        var text = $28('shortText', this).text()
        $31('tradeItemDescriptionInformation > descriptionShort').after('<descriptionShort languageCode="' + lang + '">' + text + '</descriptionShort>')
      })
      $31('tradeItemDescriptionInformation > descriptionShort').first().remove()
      
      $28('functionalName > description', this).each(function () {
        var lang = $28('languageISOCode', this).text()
        var text = $28('shortText', this).text()
        $31('tradeItemDescriptionInformation > functionalName').after('<functionalName languageCode="' + lang + '">' + text + '</functionalName>')
      })
      $31('tradeItemDescriptionInformation > functionalName').first().remove()
      
      $28('invoiceName > description', this).each(function () {
        var lang = $28('languageISOCode', this).text()
        var text = $28('shortText', this).text()
        $31('tradeItemDescriptionInformation > invoiceName').after('<invoiceName languageCode="' + lang + '">' + text + '</invoiceName>')
      })
      $31('tradeItemDescriptionInformation > invoiceName').first().remove()
    })
