...
module.exports = function (cheerio, gdsn) {
  ...
  return function (ti_28_xml) {
    ...
    $28('tradingPartnerNeutralTradeItemInformation > brandOwnerOfTradeItem').first().each(function () { // 0 or 1
    ...
    $28('tradeItem > tradeItemInformation > informationProviderOfTradeItem').first().each(function () { // 1 and only 1
      ...
      $28('informationProvider > additionalPartyIdentification').each(function () {
        var id   = $28('additionalPartyIdentificationValue', this).text()
        var type = $28('additionalPartyIdentificationType', this).text()
    ...
    $28('tradingPartnerNeutralTradeItemInformation > manufacturerOfTradeItem').each(function () { // 0..*
    ...
    var display = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > tradeItemMeasurements').attr('hasDisplayReadyPackaging') || 'NOT_APPLICABLE'
    $31('tradeItem > displayUnitInformation').append($31('<hasDisplayReadyPackaging/>').text(display))
    $31('tradeItem > displayUnitInformation').append($31('<isTradeItemADisplayUnit/>').text($28('tradeItem > isTradeItemADisplayUnit').text() || 'FALSE'))
    ...
    var variant = $28('foodAndBeverageInformation').first()
    ...
    health_related_info($28, $31, $ext)

    marketing_info($28, $31, $ext)
    ...
    safety_data_sheet($28, $31, $ext)

    sales_info($28, $31, $ext)
    
    security_tag_info($28, $31, $ext)

    sustainability_info($28, $31, $ext)

    textile_material_info($28, $31, $ext)

    trade_item_data_carrier_info($28, $31, $ext)
    
    trade_item_description_info($28, $31, $ext)

    trade_item_handling($28, $31, $ext)
    
    trade_item_hierarchy($28, $31, $ext)

    trade_item_humidity($28, $31, $ext)

    trade_item_lifespan($28, $31, $ext)

    trade_item_measurements($28, $31, $ext)

    trade_item_temperature($28, $31, $ext)
  ...
  function copy_avp ($28, $31) {
    log('copy_avp')
    var $avp = $31('avpList')
    var has_data = false
    $28('value[name]').each(function () {
      has_data = true
      $avp.append($31('<stringAVP/>').attr('attributeName', $28(this).attr('name')).text($28(this).text()))
    })
  ...
  function alcohol_info($28, $31, $ext) {
    log('alcohol_info')
  ...
  function consumer_instructions($28, $31, $ext) {
    log('consumer_instructions')
    var $info = $31('<consumerInstructions/>')
    var mod_has_data = false
    $28('tradeItemHandlingInformation > consumerUsageStorageInstructions').each(function () {
      var lang = $28('language > languageISOCode', this).text()
      var text = $28('longText', this).text()
      if (lang && text) {
  ...
  function dairy_fish_meat_poultry($28, $31, $ext) {
    ...
    $28('tradeItemMeasurements > casingTareWeight').each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
      $info.append($31('<casingTareWeight/>').attr('measurementUnitCode', uom).text(value))
    })
  ...
  function delivery_info($28, $31, $ext) {
    log('delivery_info')
  ...
  function farming_info($28, $31, $ext) {
    log('farming info')

    var agency = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > organicTradeItemCharacteristics > organicClaimAgency').text()
    var code   = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > organicTradeItemCharacteristics > organicTradeItemCode').text()
  ...
  function food_and_bev_preparation($28, $31, $ext, variant) {
    ...
    $28('foodAndBeverageMarketingInformationExtension > servingSuggestion > description').each(function () {
      var lang = $28('language > languageISOCode', this).text()
      var text = $28('longText', this).text()
      if (!$prep) $prep = $31('<preparationServing/>')
  ...
  function health_related_info($28, $31, $ext) {
    log('health_related_info')
    var mod_has_data = false
    var $info = $31('<healthRelatedInformation/>')
    $28('nutritionLabelTypeCode').each(function () {
      mod_has_data = true
      $info.append($31('<nutritionalLabelTypeCode/>').text($28(this).text()))
    })
    /*
    var cons = $28('tradeItemSustainabilityInformation > tradeItemEnvironmentalProperties').attr('isTradeItemChemicalNotIntendedForHumanConsumption')
    if (cons) {
      mod_has_data = true
  ...
  function health_wellness_pack($28, $31, $ext) {
    log('health_wellness_pack')
    var mod_has_data = false
    var $info = $31('<healthWellnessPackagingMarking/>')
    $28('packagingMarking').first().each(function () {
      mod_has_data = true
      $info.append($31('<isPackagingMarkedWithIngredients/>').text($28('isPackagingMarkedWithIngredients', this).text()))
  ...
  function marketing_info($28, $31, $ext) {
    log('marketing_info')
    var feature = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > marketingInformation > tradeItemFeatureBenefit').text()
    if (!feature) return
    ...
    $info.append($31('<tradeItemFeatureBenefit/>').attr('languageCode', 'en').text(feature))
    $mod.append($info)
    $ext.append($mod)
  } // end marketing_info
  ...
  function nutrient_info($28, $31, $ext, variant) {
    ...
    $28('foodAndBeverageNutrientInformation', variant).each(function () {
      ...
      $28('servingSize > measurementValue', this).first().each(function () {
        var value = $28('value', this).text()
        var uom = $28(this).attr('unitOfMeasure')
        $header.append($31('<servingSize/>').attr('measurementUnitCode', uom).text(value))
      })
  ...
  function packaging_info($28, $31, $ext) {
    log('packaging_info')
    var mod_has_data = false
    var $info = $31('<packaging/>')
    var description
    $info.append($31('<doesPackagingHaveWheels/>').text('FALSE'))
    $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > packagingType').first().each(function () {
      ...
      $28('packagingWeight', this).each(function () {
        var value = $28('value', this).text()
        var uom = $28(this).attr('unitOfMeasure')
        $info.append($31('<packagingWeight/>').attr('measurementUnitCode', uom).text(value))
      })
      description = $28('packagingTypeDescription', this).text()
    })
    $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > packagingMaterial').first().each(function () {
      ...
      $28('packagingMaterialCompositionQuantity > measurementValue', this).each(function () {
        var uom   = $28(this).attr('unitOfMeasure')
        var value = $28('value', this).text()
        $material.append($31('<packagingMaterialCompositionQuantity/>').attr('measurementUnitCode', uom).text(value))
      })

      var dep_eff_date = $28('depositValueEffectiveDate', this).text()
      var dep_end_date = $28('depositValueEndDate', this).text()
      if (dep_eff_date || dep_end_data) {
        ...
        $deposit.append($31('<returnablePackageDepositAmount/>').test($28('returnablePackageDepositAmount', this).text()))
  ...
  function packaging_marking($28, $31, $ext) {
    ...
    $28('packagingMarking').first().each(function () {
      ...
      var net_content = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > tradeItemMeasurements > isNetContentDeclarationIndicated').text()
      ...
      var acc_code = $28('packageMarksEnvironment').text()
      if (acc_code) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(acc_code))
      acc_code = $28('packageMarksEthical').text()
      if (acc_code) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(acc_code))
      acc_code = $28('packageMarksHygienic').text()
      if (acc_code) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(acc_code))
  ...
  function safety_data_sheet($28, $31, $ext) {
    log('safety_data_sheet')
    var msds = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > materialSafetyData > materialSafetyDataSheetNumber').text()
    if (!msds) return
  ...
  function sales_info($28, $31, $ext) {
    ...
    var pcc = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > tradeItemMeasurements > priceComparisonContentType').text()
    ...
    $28('tradeItemMeasurements > priceComparisonMeasurement > measurementValue').each(function () {
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
      $info.append($31('<priceComparisonMeasurement/>').attr('measurementUnitCode', uom).text(value))
    })
  ...
  function security_tag_info($28, $31, $ext) {
    log('security_tag_info')
    var loc = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > securityTagInformation > securityTagLocation').text()
    var typ = $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > securityTagInformation > securityTagType').text()
    if (!loc && !typ) return
  ...
  function sustainability_info($28, $31, $ext) {
    log('sustainability_info')
    var mod_has_data = false
    var $info = $31('<sustainabilityInformation/>')
    $28('tradingPartnerNeutralTradeItemInformation > tradeItemSustainabilityInformation').first().each(function () {
  ...
  function textile_material_info($28, $31, $ext) {
    ...
    var $info
    $28('tradingPartnerNeutralTradeItemInformation > materialComposition').each(function () {
      ...
      $28('tradingPartnerNeutralTradeItemInformation > tradeItemMaterial').first().each(function () {
        ...
        $comp.append($31('<materialWeight/>').attr('measurementUnitCode', uom).text(value))
      })
  ...
  function trade_item_data_carrier_info($28, $31, $ext) {
    ...
    $28('tradingPartnerNeutralTradeItemInformation > tradeItemBarCodeInformation > barCodeInformation').each(function () {
      mod_has_data = true
      var value     = $28('barCodeValue', this).text()
      var type_code = $28('barCodeValueType', this).text()
      var $key  = $31('<gs1TradeItemIdentificationKey/>')
      $key.append($31('<gs1TradeItemIdentificationKeyCode/>').text(type_code))
    ...
    $28('tradingPartnerNeutralTradeItemInformation > barCodeType').each(function () {
      mod_has_data = true
      $mod.append($31('<dataCarrier/>').append($31('<dataCarrierTypeCode/>').text($28(this).text())))
    })
    if (mod_has_data) $ext.append($mod)
  } // end trade_item_data_carrier_info

  function trade_item_description_info($28, $31, $ext) {
  ...
  function trade_item_handling($28, $31, $ext) {
    log('trade_item_handling')
    var mod_has_data = false
    var $info = $31('<tradeItemHandlingInformation/>')
    $28('tradeItemHandlingInformation').each(function () {
      $28('clampPressure', this).first().each(function () {
        mod_has_data = true
        var uom   = $28(this).attr('unitOfMeasure')
        var value = $28('value', this).text()
        $info.append($31('<clampPressure/>').attr('measurementUnitCode', uom).text(value))
      })
      ...
      $28('stackingWeightMaximum > measurementValue', this).first().each(function () {
        ...
        var value = $28('value', this).text()
        var uom = $28(this).attr('unitOfMeasure')
        $stack.append($31('<stackingWeightMaximum/>').attr('measurementUnitCode', uom).text(value))
      })
    })
    if (!mod_has_data) return
  ...
  function trade_item_hierarchy($28, $31, $ext) {
    log('trade_item_hierarchy')
    var $info = $31('<tradeItemHierarchy/>')
    $28('tradeItemHierarchy').first().each(function () {
      mod_has_data = true
      $info.append($31('<isTradeItemPackedIrregularly/>').text($28('isTradeItemPackedIrregularly', this).text() || 'UNSPECIFIED'))
      $28('tradeItemMeasurements > layerHeight').each(function () {
        var uom   = $28(this).attr('unitOfMeasure')
        var value = $28('value', this).text()
        $info.append($31('<layerHeight/>').attr('measurementUnitCode', uom).text(value))
      })
  ...
  function trade_item_humidity($28, $31, $ext) {
    ...
    $28('tradingPartnerNeutralTradeItemInformation > tradeItemTemperatureInformation').each(function () {
  ...
  function trade_item_lifespan($28, $31, $ext) {
    log('trade_item_lifespan')
    var arrival
    var product
    var opened

    $28('tradingPartnerNeutralTradeItemInformation > tradeItemHandlingInformation').each(function () {
  ...
  function trade_item_measurements($28, $31, $ext) {
    ...
    $28('tradeItemMeasurements > individualUnitMaximumSize').each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
    ...
    $28('tradeItemMeasurements > individualUnitMinimumSize').each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
    ...
    $28('tradeItemMeasurements > tradeItemCompositionDepth > measurementValue').first().each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
      $info.append($31('<tradeItemCompositionDepth/>').attr('measurementUnitCode', uom).text(value))
    })
    
    $28('tradeItemMeasurements > tradeItemCompositionWidth > measurementValue').first().each(function () {
      mod_has_data = true
      var uom   = $28(this).attr('unitOfMeasure')
      var value = $28('value', this).text()
      $info.append($31('<tradeItemCompositionWidth/>').attr('measurementUnitCode', uom).text(value))
    })
    ...
    $28('tradeItemMeasurements > pegMeasurements').each(function () {
      mod_has_data = true
      ...
      $28('pegHorizontal > measurementValue', this).first().each(function () {
        var uom   = $28(this).attr('unitOfMeasure')
        var value = $28('value', this).text()
        $peg.append($31('<pegHorizontal/>').attr('measurementUnitCode', uom).text(value))
      })
      $28('pegVertical > measurementValue', this).first().each(function () {
        var uom   = $28(this).attr('unitOfMeasure')
        var value = $28('value', this).text()
        $peg.append($31('<pegVertical/>').attr('measurementUnitCode', uom).text(value))
      })
      $info.append($peg)
    })
    
    var $nest = $31('<tradeItemNesting/>')
    $28('tradeItemMeasurements > tradeItemNesting').first().each(function () {
      mod_has_data = true
      $nest.append($31('<nestingDirectionCode/>').text($28('nestingDirection', this).text() || 'na'))
      $28('tradeItemMeasurements > nestingIncrement> measurementValue').each(function () {
        var uom   = $28(this).attr('unitOfMeasure')
        var value = $28('value', this).text()
        $nest.append($31('<nestingIncrement/>').attr('measurementUnitCode', uom).text(value))
      })
  ...
  function trade_item_temperature($28, $31, $ext) {
    ...
    $28('tradingPartnerNeutralTradeItemInformation > tradeItemTemperatureInformation').each(function () {
      ...
      $28('deliveryToDistributionCenterTemperatureMaximum > measurementValue', this).first().each(function () {
        ...
        $info.append($31('<maximumTemperature/>').attr('temperatureMeasurementUnitCode', $28(this).attr('unitOfMeasure')).text($28('value', this).text()))
      })
      $28('deliveryToDistributionCenterTemperatureMinimum > measurementValue', this).first().each(function () {
        if ($info) $info.append($31('<minimumTemperature/>').attr('temperatureMeasurementUnitCode', $28(this).attr('unitOfMeasure')).text($28('value', this).text()))
      })
      if ($info) {
  ...
  function variable_info($28, $31, $ext) {
    log('variable_info')
    ...
    var text = $28('tradeItemUnitIndicator > isTradeItemAVariableUnit').text()
    if (!text) return

    var $info = $31('<variableTradeItemInformation/>')
    $info.append($31('<isTradeItemAVariableUnit/>').text(text))

    var code = $28('tradeItemUnitIndicator > variableTradeItemType').text()
  ...
  function create_additional_variants($28, $31) { // additional variants can only have allergen, diet, ingredient, and nutrient info
