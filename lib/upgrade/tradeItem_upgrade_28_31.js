...
var BOOL = function (s) {
  if (!s || s == 'FALSE' || s == 'False' || s == 'false' || s == 'NO' || s == 'No' || s == 'no') return ''
  else return 'true'
}
...
module.exports = function (cheerio, gdsn) {
  ...
  function convert_ti_xml(xml) {
    ...
    var avp_prelim = $28('value[name="preliminaryStatusCode"]', ti28).text()
    if (avp_prelim) $31('tradeItem > tradeItemUnitDescriptorCode').before($31('<preliminaryItemStatusCode/>').text(avp_prelim))
    ...
    var avp_dt = $28('value[name="displayTypeCode"]', ti28).text()
    if (avp_dt) {
      $31('tradeItem > displayUnitInformation').append($31('<displayTypeCode/>').text(avp_dt))
    }
    ...
    $28('classificationCategoryCode', ti28).first().each(function () {
        ...
        var name = $28('classificationCategoryCode > classificationCategoryName', this).first().text()
        if (name) $31('gdsnTradeItemClassification > gpcCategoryName').text(name)
        else $31('gdsnTradeItemClassification > gpcCategoryName').remove()
    })
    ...
    controlled_substance($28, ti28, $31, $ext)
    ...
    food_and_bev_preparation($28, ti28, $31, $ext, variant)
    ...
    nutrient_info($28, ti28, $31, $ext, variant)

    packaging_info($28, ti28, $31, $ext)

    packaging_marking($28, ti28, $31, $ext)

    place_of_item_activity($28, ti28, $31, $ext)

    referenced_file($28, ti28, $31, $ext)

    regulated_trade_item($28, ti28, $31, $ext)
  ...
  function copy_avp ($28, ti28, $31) {
    ...
    $28('value[name]', ti28).each(function () {
      ...
      // migrate to specific 3.1 target tag and remove from avp list:
      if (name == 'alcoholicBeverageSubregionOfOrigin')        return // alcoholInformation/alcoholicBeverageSubregion
      if (name == 'doesTradeItemCarryUSDAChildNutritionLabel') return // healthRelatedInformationModule nutritionalLabelTypeCode 1
      if (name == 'extendedTradeItemDateOnPackagingTypeCode')  return // packagingMarkingModule/packagingMarking/packagingDate/tradeItemDateOnPackagingTypeCode
      if (name == 'isTradeItemManagedByManufactureDate')       return // packagingMarkingModule/packagingMarking/packagingDate/tradeItemDateOnPackagingTypeCode=PRODUCTION_DATE
      if (name == 'packagingMarkedLabelAccreditationCode')     return // packagingMarkingModule/packagingMarking/packagingMarkedLabelAccreditationCode
      if (name == 'isTradeItemBiodegradable')                  return // sustainabilityModule/tradeItemSustainabilityFeatureCode
      if (name == 'consumerUsageInstructions')                 return // consumerInstructionsModule consumerStorageInstructions
      if (name == 'isPrivateLabel')                            return // salesInformationModule brandDistributionTypeCode PRIVATE_LABEL_DISTRIBUTION
      if (name == 'isControlledSubstance')                     return // controlledSubstanceModule/doesItemContainAControlledSubstance=TRUE
      if (name == 'regulatedProductName')                      return // tradeItemDescriptionModule/tradeItemDescriptionInformation/regulatedProductName 
      if (name == 'isTradeItemAFoodstuff')                     return // regulatedTradeItemModule/regulatoryInformation/regulationTypeCode=FOODSTUFF_DIRECTIVE
      if (name == 'labelDescription')                          return // tradeItemDescriptionModule/tradeItemDescriptionInformation/labelDescription
      if (name == 'suggestedConsumerPack')                     return // tradeItemHierarchyModule/tradeItemHierarchy/suggestedConsumerPack
      if (name == 'individualUnitMax')                         return // tradeItemMeasurementsModule
      if (name == 'individualUnitMaxUOM')                      return // tradeItemMeasurementsModule
      if (name == 'individualUnitMin')                         return // tradeItemMeasurementsModule
      if (name == 'individualUnitMinUOM')                      return // tradeItemMeasurementsModule
      if (name == 'tradeItemCubeMeasurement')                  return // tradeItemMeasurementsModule inBoxCubeDimension
      if (name == 'tradeItemCubeMeasurementUOM')               return // tradeItemMeasurementsModule inBoxCubeDimension measurementUnitCode
      if (name == 'foodCompositionAgencyNameCode')             return // nutritionalInformationModule/foodBeverageComposition/foodBeverageCompositionDatabaseCode
      if (name == 'nutritionalClaimTypeCode')                  return // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimTypeCode
      if (name == 'nutritionalClaimNutritionElement')          return // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimNutritionElement
      if (name == 'isNutritionalClaimCodeTransFatFree')        return // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimTypeCode=FREE_FROM
                                                                      // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimNutritionElement=TRANS_FAT
      if (name == 'isNutritionalClaimCodeLowCholesterol')      return // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimTypeCode=LOW
                                                                      // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimNutritionElement=CHOLESTEROL
      if (name == 'isNutritionalClaimCodeReducedFat')          return // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimTypeCode=REDUCED
                                                                      // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimNutritionElement=FAT
      if (name == 'isNutritionalClaimCodeNatural')             return // nutritionalInformationModule/nutritionalClaimDetail/nutritionalClaimTypeCode=NATURAL
      if (name == 'isPackageMarkedWithLactoseContainment')     return // healthWellnessPackagingMarkingModule/healthWellnessPackagingMarking/packagingMarkedFreeFromCode=REDUCED_LACTOSE

      if (name == 'numberOfServingsPerPackageDecimal')                 return // foodAndBeveragePreparationServingModule/servingQuantityInformation/numberOfServingsPerPackage
      if (name == 'measurementPrecisionOfNumberOfServingsPerPackage')  return // foodAndBeveragePreparationServingModule/servingQuantityInformation/numberOfServingsPerPackageMeasurementPrecisionCode
      if (name == 'unspecifiedPreparationTypePreparationInstructions') return // foodAndBeveragePreparationServingModule/preparationServing/preparationInstructions

      if (name == 'fatPercentageInDryMatter') return // dairyFishMeatPoultryItemModule/cheeseInformation/
      if (name == 'displayTypeCode')          return // tradeItem/displayUnitInformation/
      if (name == 'countryOfOriginStatement') return // placeOfItemActivityModule/countryOfOrigin/countryCode
      if (name == 'placeOfProvenance')        return // placeOfItemActivityModule/
  ...
  function alcohol_info($28, ti28, $31, $ext, variant) {
    ...
    var avp_ori = $28('value[name="alcoholicBeverageSubregionOfOrigin"]', ti28).text()
    if (avp_ori) {
      mod_has_data = true
      $info.append($31('<alcoholicBeverageSubregion/>').text(avp_ori))
    }
  ...
  function consumer_instructions($28, ti28, $31, $ext) {
    ...
    if (!mod_has_data) { // check for AVP alternate
      var avp_con = $28('value[name="consumerUsageInstructions"]', ti28).text()
      if (avp_con) {
        $info.append($31('<consumerStorageInstructions/>').attr('languageCode', 'en').text(avp_con))
    mod_has_data = true
      }
    }
  ...
  function controlled_substance($28, ti28, $31, $ext) {
    log('controlled_substance')
    var mod_has_data = false
    var $mod = $31('<controlled_substance:controlledSubstanceModule xmlns:controlled_substance="urn:gs1:gdsn:controlled_substance:xsd:3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:gs1:gdsn:controlled_substance:xsd:3 http://www.gdsregistry.org/3.1/schemas/gs1/gdsn/ControlledSubstanceModule.xsd"/>')

    if (BOOL($28('value[name="isControlledSubstance"]', ti28).text())) {
      $mod.append($31('<doesItemContainAControlledSubstance/>').text('TRUE'))
      mod_has_data = true
    }
    if (!mod_has_data) return
    $ext.append($mod)
  } // end controlled_substance 
  ...
  function dairy_fish_meat_poultry($28, ti28, $31, $ext) {
    ...
    var avp = $28('value[name="fatPercentageInDryMatter"]', ti28).text()
    if (avp) {
      mod_has_data = true
      $info.append($31('<cheeseInformation/>').append($31('<fatPercentageInDryMatter/>').text(avp)))
    }
    if (mod_has_data) $ext.append($mod)
  ...
  function food_and_bev_preparation($28, ti28, $31, $ext, variant) {
    ...
    if (!num_serv) num_serv = $28('value[name="numberOfServingsPerPackageDecimal"]', ti28).text() // ti AVP copied to each variant
    if (num_serv) {
      var $info = $31('<servingQuantityInformation/>')
      $mod.append($info)
      mod_has_data = true

      $info.append($31('<numberOfServingsPerPackage/>').text(num_serv))

      var avp_prec = $28('value[name="measurementPrecisionOfNumberOfServingsPerPackage"]', ti28).text() // ti AVP copied to each variant
      if (avp_prec) $info.append($31('<numberOfServingsPerPackageMeasurementPrecisionCode/>').text(avp_prec))
    }
    $28('foodAndBeveragePreparationInformation', variant).each(function () {
      var $prep = $31('<preparationServing/>')
      $28('preparationInstructions > description', this).each(function () {
        var lang = $28('language > languageISOCode', this).text()
        var text = $28('text', this).text()
        $prep.append($31('<preparationInstructions/>').attr('languageCode', lang).text(text))
      })
      var prep_type = $28('preparationType', this).text() || 'UNSPECIFIED'
      $prep.append($31('<preparationTypeCode/>').text(prep_type))
      $mod.append($prep)
      mod_has_data = true
    })

    var prep_un = $28('value[name="unspecifiedPreparationTypePreparationInstructions"]', ti28).text() // ti AVP copied to each variant
    if (prep_un) {
      var $prep = $31('<preparationServing/>')
      $prep.append($31('<preparationInstructions/>').attr('languageCode', 'en').text(prep_un))
      $prep.append($31('<preparationTypeCode/>').text('UNSPECIFIED'))
      $mod.append($prep)
      mod_has_data = true
    }

    $28('foodAndBeverageMarketingInformationExtension > servingSuggestion', ti28).first().each(function () {
      var $prep = $31('<preparationServing/>')
      $prep.append($31('<preparationTypeCode/>').text('UNSPECIFIED'))
      $28('description', this).each(function () {
        var lang = $28('language > languageISOCode', this).text()
        var text = $28('longText', this).text()
  ...
  function health_related_info($28, ti28, $31, $ext) {
    ...
    var avp_child = $28('value[name="doesTradeItemCarryUSDAChildNutritionLabel"]', ti28).text()
    if (BOOL(avp_child)) {
      mod_has_data = true
      $info.append($31('<nutritionalLabelTypeCode/>').text('1'))
    }
  ...
  function health_wellness_pack($28, ti28, $31, $ext) {
    ...
    var avp_lact = $28('value[name="isPackageMarkedWithLactoseContainment"]', ti28).text()
    if (BOOL(avp_lact)) {
      mod_has_data = true
      $info.append($31('<packagingMarkedFreeFromCode/>').text('REDUCED_LACTOSE'))
    }
    if (!mod_has_data) return
  ...
  function marketing_info($28, ti28, $31, $ext) {
    ...
    $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > marketingInformation', ti28).each(function () {
      ...
      var avp_ben = $28('value[name="tradeItemFeatureBenefitExpanded"]', ti28).text()
      if (avp_ben) {
        mod_has_data = true
        $info.append($31('<tradeItemFeatureBenefit/>').attr('languageCode', 'en').text(avp_ben))
      }
  ...
  function nutrient_info($28, ti28, $31, $ext, variant) {
    if (!variant) return
    log('nutrient_info')
    ...
    $28('foodAndBeverageMarketingInformationExtension > nutritionalClaimCode', ti28).each(function () {
      log('hello ' + this)
      mod_has_data = true
      $mod.append($31('<nutritionalClaimDetail/>').append($31('<nutritionalClaimTypeCode/>').text($28(this).text())))
    })

    var avp_comp = $28('value[name="foodCompositionAgencyNameCode"]', ti28).text()
    if (avp_comp) {
      mod_has_data = true
      var $comp = $31('<foodBeverageComposition/>')
      if (avp_comp) $comp.append($31('<foodBeverageCompositionDatabaseCode/>').text(avp_comp))
      $mod.append($comp)
    }

    var avp_claim = $28('value[name="nutritionalClaimTypeCode"]', ti28).text()
    var avp_elem  = $28('value[name="nutritionalClaimNutritionElement"]', ti28).text()
    if (avp_claim || avp_elem) {
      mod_has_data = true
      var $claim = $31('<nutritionalClaimDetail/>')
      if (avp_claim) $claim.append($31('<nutritionalClaimTypeCode/>').text(avp_claim))
      if (avp_elem)  $claim.append($31('<nutritionalClaimNutrientElementCode/>').text(avp_elem))
      $mod.append($claim)
    }

    var avp_claim_tf = $28('value[name="isNutritionalClaimCodeTransFatFree"]', ti28).text()
    if (BOOL(avp_claim_tf)) {
      mod_has_data = true
      var $claim_tf = $31('<nutritionalClaimDetail/>')
      $claim_tf.append($31('<nutritionalClaimTypeCode/>').text('FREE_FROM'))
      $claim_tf.append($31('<nutritionalClaimNutrientElementCode/>').text('TRANS_FAT'))
      $mod.append($claim_tf)
    }

    var avp_claim_ch = $28('value[name="isNutritionalClaimCodeLowCholesterol"]', ti28).text()
    if (BOOL(avp_claim_ch)) {
      mod_has_data = true
      var $claim_ch = $31('<nutritionalClaimDetail/>')
      $claim_ch.append($31('<nutritionalClaimTypeCode/>').text('LOW'))
      $claim_ch.append($31('<nutritionalClaimNutrientElementCode/>').text('CHOLESTEROL'))
      $mod.append($claim_ch)
    }

    var avp_claim_rf = $28('value[name="isNutritionalClaimCodeReducedFat"]', ti28).text()
    if (BOOL(avp_claim_rf)) {
      mod_has_data = true
      var $claim_rf = $31('<nutritionalClaimDetail/>')
      $claim_rf.append($31('<nutritionalClaimTypeCode/>').text('REDUCED'))
      $claim_rf.append($31('<nutritionalClaimNutrientElementCode/>').text('FAT'))
      $mod.append($claim_rf)
    }

    var avp_claim_nt = $28('value[name="isNutritionalClaimCodeNatural"]', ti28).text()
    if (BOOL(avp_claim_nt)) {
      mod_has_data = true
      var $claim_nt = $31('<nutritionalClaimDetail/>')
      $claim_nt.append($31('<nutritionalClaimTypeCode/>').text('NATURAL'))
      $mod.append($claim_nt)
    }
  ...
  function packaging_marking($28, ti28, $31, $ext) {
    ...
    $28('packagingMarking', ti28).first().each(function () {
      ...
      var value = $28('value[name="packagingMarkedLabelAccreditationCode"]', ti28).text()
      if (value) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(value))

      value = $28('packageMarksEnvironment', this).text()
      if (value) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(value))

      value = $28('packageMarksEthical', this).text()
      if (value) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(value))

      value = $28('packageMarksHygienic', this).text()
      if (value) $info.append($31('<packagingMarkedLabelAccreditationCode/>').text(value))

      $info.append($31('<packagingMarkedRecyclableScheme/>').text($28('packagingMarkedRecyclableScheme', this).text() || 'UNSPECIFIED'))

      var pack_date_type = $28('packagingMarkedExpirationDateType', this).text()
      if (!pack_date_type) pack_date_type = $28('value[name="extendedTradeItemDateOnPackagingTypeCode"]', ti28).text()
      if (!pack_date_type && BOOL($28('value[name="isTradeItemManagedByManufactureDate"]', ti28).text())) pack_date_type = 'PRODUCTION_DATE'
  ...
  function place_of_item_activity($28, ti28, $31, $ext) {
    log('place_of_item_activity')
    var mod_has_data = false
    var $info = $31('<placeOfProductActivity/>')

    var orig_stat = $28('value[name="countryOfOriginStatement"]', ti28).text()
    if (orig_stat) {
      $info.append($31('<countryOfOriginStatement/>').attr('languageCode', 'en').text(orig_stat))
      mod_has_data = true
    }
    var prov = $28('value[name="placeOfProvenance"]', ti28).text()
    if (prov) {
      $info.append($31('<provenanceStatement/>').attr('languageCode', 'en').text(prov))
      mod_has_data = true
    }
    var origin = $28('tradeItemCountryOfOrigin > countryISOCode', ti28).text()
    if (origin) {
      $info.append($31('<countryOfOrigin/>').append($31('<countryCode/>').text(origin)))
      mod_has_data = true
    }
    if (!mod_has_data) return
    var $mod = $31('<place_of_item_activity:placeOfItemActivityModule xmlns:place_of_item_activity="urn:gs1:gdsn:place_of_item_activity:xsd:3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:gs1:gdsn:place_of_item_activity:xsd:3 http://www.gdsregistry.org/3.1/schemas/gs1/gdsn/PlaceOfItemActivityModule.xsd"/>')
    $mod.append($info)
    $ext.append($mod)
  } // end place_of_item_activity

  function referenced_file($28, ti28, $31, $ext) {
    log('referenced_file')
    var mod_has_data = false
    var $mod = $31('<referenced_file_detail_information:referencedFileDetailInformationModule xmlns:referenced_file_detail_information="urn:gs1:gdsn:referenced_file_detail_information:xsd:3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:gs1:gdsn:referenced_file_detail_information:xsd:3 http://www.gdsregistry.org/3.1/schemas/gs1/gdsn/ReferencedFileDetailInformationModule.xsd"/>')
    $28('tradeItemExternalInformation', ti28).each(function () { // will include food and bev images!
      ...
      var ref_file = $31('<referencedFileHeader/>')
  ...
  function regulated_trade_item($28, ti28, $31, $ext) {
    log('regulated_trade_item')
    var avp_food = $28('value[name="isTradeItemAFoodstuff"]', ti28).text()
    if (!BOOL(avp_food)) return
    var $mod = $31('<regulated_trade_item:regulatedTradeItemModule xmlns:regulated_trade_item="urn:gs1:gdsn:regulated_trade_item:xsd:3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:gs1:gdsn:regulated_trade_item:xsd:3 http://www.gdsregistry.org/3.1/schemas/gs1/gdsn/RegulatedTradeItemModule.xsd"/>')
    var $info = $31('<regulatoryInformation/>')
    $info.append($31('<regulationTypeCode/>').text('FOODSTUFF_DIRECTIVE'))
    $mod.append($info)
    $ext.append($mod)
  } // end regulated_trade_item
  ...
  function sales_info($28, ti28, $31, $ext) {
    ...
    if (!bdt && BOOL($28('value[name="isPrivateLabel"]', ti28).text())) bdt = 'PRIVATE_LABEL_DISTRIBUTION'
  ...
  function sustainability_info($28, ti28, $31, $ext) {
    ...
    $28('tradingPartnerNeutralTradeItemInformation > tradeItemSustainabilityInformation', ti28).first().each(function () {
      ...
      var avp_bio = $28('value[name="isTradeItemBiodegradable"]', ti28).text()
      if (BOOL(avp_bio)) {
        mod_has_data = true
  ...
  function trade_item_data_carrier_info($28, ti28, $31, $ext) {
    ...
    if (!has_bct) $mod.append($31('<dataCarrier/>').append($31('<dataCarrierTypeCode/>').text('NO_BARCODE')))
  ...
  function trade_item_description_info($28, ti28, $31, $ext) {
    ...
    $28('tradeItemDescriptionInformation', ti28).first().each(function () { // should be only one
      ...
      var avp_ld = $28('value[name="labelDescription"]', ti28).text()
      if (avp_ld) $info.append($31('<labelDescription/>').attr('languageCode', 'en').text(avp_ld))

      var productRange = $28('productRange', this).text()
      if (productRange) $info.append($31('<productRange/>').text(productRange))

      var rpn = $28('value[name="regulatedProductName"]', ti28).text()
      if (rpn) $info.append($31('<regulatedProductName/>').attr('languageCode', 'en').text(rpn))
      ...
      var brandName = $28('brandName', this).text()
      log('>>>>>>>>>>>>>>>>>> brandName: ' + brandName)

      $brand.append($31('<brandName/>').text(brandName)) 

      //$brand.append($31('<brandName>' + brandName + '</brandName>')) // works with decodeEntities true or false!

      // works!: $brand.append($31('<brandName><![CDATA[' + brandName + ']]></brandName>'))

      //$brand.append($31('<brandName/>').text(function (idx, str) { 
          //log('fn args: ' + Array.prototype.join.call(arguments))
          //return decodeURIComponent(brandName)
          //return 'N © 05'
      //}))

      log('brand text(): ' + $brand.text())
      log('brand html(): ' + $brand.html())
  ...
  function trade_item_hierarchy($28, ti28, $31, $ext) {
    ...
    $28('tradeItemHierarchy', ti28).first().each(function () {
      mod_has_data = true
      var irr = $28(this).attr('isTradeItemPackedIrregularly')
      $info.append($31('<isTradeItemPackedIrregularly/>').text(irr || 'FALSE'))
      ...
      var layers_per_item = $28('quantityOfCompleteLayersContainedInATradeItem', this).text()
      if (layers_per_item) $info.append($31('<quantityOfCompleteLayersContainedInATradeItem/>').text(layers_per_item))
      ...
      var avp_pack = $28('value[name="suggestedConsumerPack"]', ti28).text()
      if (avp_pack) $info.append($31('<suggestedConsumerPack/>').text(avp_pack))
    })
    if (!mod_has_data) return
  ...
  function trade_item_measurements($28, ti28, $31, $ext) {
    ...
    $28('tradeItemMeasurements', ti28).first().each(function () {
      ...
      if (!has_cube) { // check for AVP alternate
        var avp_cube    = $28('value[name="tradeItemCubeMeasurement"]', ti28).text()
        var avp_cubeUom = $28('value[name="tradeItemCubeMeasurementUOM"]', ti28).text()
        if (avp_cube && avp_cubeUom) {
          mod_has_data = true
          $info.append($31('<inBoxCubeDimension/>').attr('measurementUnitCode', UOM(avp_cubeUom)).text(avp_cube))
        }
      }
      ...
      if (!iuMax) { // check for AVP alternate
        var avp_im    = $28('value[name="individualUnitMax"]', ti28).text()
        var avp_imUom = $28('value[name="individualUnitMaxUOM"]', ti28).text()
        if (avp_im && avp_imUom) {
          mod_has_data = true
          $info.append($31('<individualUnitMaximumSize/>').attr('measurementUnitCode', UOM(avp_imUom)).text(avp_im))
        }
      }
      ...
      if (!iuMin) { // check for AVP alternate
    var avp_ix    = $28('value[name="individualUnitMin"]', ti28).text()
    var avp_ixUom = $28('value[name="individualUnitMinUOM"]', ti28).text()
    if (avp_ix && avp_ixUom) {
      mod_has_data = true
          $info.append($31('<individualUnitMinimumSize/>').attr('measurementUnitCode', UOM(avp_ixUom)).text(avp_ix))
    }
      }
      ...
      $28('tradeItemNesting', this).first().each(function () {
        mod_has_data = true
        $nest.append($31('<nestingDirectionCode/>').text($28('nestingDirection', this).text() || 'na')) // default?
        ...
        $nest.append($31('<nestingTypeCode/>').text($28('nestingType', this).text() || 'na')) // default?
        $info.append($nest)
      })
  ...
  function create_additional_variants($28, ti28, $31) { // additional variants can only have allergen, diet, ingredient, and nutrient info
    ...
    $28('foodAndBeverageInformation', ti28).each(function () {
      ...
      food_and_bev_preparation($28, ti28, $31, $ext, this)

      nutrient_info($28, ti28, $31, $ext, this)

    }) // end each additional 2.8 foodAndBeverageInformation
  } // end additional production variants

  function contact_info($28, ti28, $31) {
    log('contact_info')
    $28('tradeItemInformation > tradeItemContactInformation', ti28).each(function () {
      var $info = $31('<tradeItemContactInformation/>')
      $31('tradeItem > targetMarket').after($info)

      $info.append($31('<contactTypeCode/>').text($28('contactType', this).text()))

      $28('availableTime', this).first().each(function () {
        var lang = $28('language > languageISOCode', this).text()
        var text = $28('text', this).text()
        if (lang && text) $info.append($31('<availableTime/>').attr('languageCode', lang).text(text))
      })

      $info.append($31('<gln/>').text($28('partyIdentification > gln', this).text()))

      $28('targetMarketCommunicationChannel', this).each(function () {
        var $tmc = $31('<targetMarketCommunicationChannel/>')
        $info.append($tmc)

        var tm = $28('targetMarket > targetMarketCountryCode > countryISOCode', this).text()
        if (tm) $tmc.append($31('<targetMarket/>').append($31('<targetMarketCountryCode/>').text(tm)))

        $28('communicationChannel', this).each(function () {
          var code = $28(this).attr('communicationChannelCode')
          var numb = $28(this).attr('communicationNumber')
          if (!code && !numb) return
          var chan = $31('<communicationChannel/>')
          if (code) chan.append($31('<communicationChannelCode/>').text(code))
          if (numb) chan.append($31('<communicationValue/>').text(numb))
          $tmc.append(chan)
        })
      }) // end each targetMarketCommunicationChannel

    }) // end each contactInfo
  } // end contact_info 
}
