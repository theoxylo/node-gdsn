...
module.exports = function (cheerio, gdsn) {
  ...
  function marketing_info($28, $31, $ext) {
    ...
    $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > marketingInformation').each(function () {
      mod_has_data = true

      var feature = $28('tradeItemFeatureBenefit', this).text()
      if (feature) $info.append($31('<tradeItemFeatureBenefit/>').attr('languageCode', 'en').text(feature))

      var $campaign = $31('<marketingCampaign/>')
      $info.append($campaign)

      var end = $28('campaignEndDate', this).text()
      if (end) $campaign.append($31('<campaignEndDateTime/>').text(end + 'T01:01:01.009Z'))

      var lang = $28('campaignName > language > languageISOCode', this).text()
      var text = $28('campaignName > text', this).text()
      $campaign.append($31('<campaignName/>').attr('languageCode', lang).text(text))

      var start = $28('campaignStartDate', this).text()
      if (start) $campaign.append($31('<campaignStartDateTime/>').text(start + 'T01:01:01.009Z'))
    })

    // season
    $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > season').each(function () {
      mod_has_data = true
  ...
  function packaging_info($28, $31, $ext) {
    ...
    $28('tradeItem > tradeItemInformation > tradingPartnerNeutralTradeItemInformation > tradeItemPalletInformation').first().each(function () {
      var code = $28('palletTypeCode', this).text()
      if (!code) return
      var $pallet = $31('<packaging/>')
      $pallet.append($31('<platformTermsAndConditionsCode/>').text($28('palletTermsAndConditions', this).text()))
