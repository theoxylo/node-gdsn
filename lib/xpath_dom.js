...
module.exports = function addDomSupport(gdsn) {
  ...
  gdsn.dom.processCinFromOtherDp = function (cinInFile, cb) {
    ...
    var ts = Date.now()
    var responseOutFile = gdsn.opts.outbox_dir + '/out_cin_response_to_other_dp_'   + ts + '.xml'
    var forwardOutFile  = gdsn.opts.outbox_dir + '/out_cin_forward_to_local_party_' + ts + '.xml'
  ...
  gdsn.dom.createCinResponse = function ($cin, cb) {
    setImmediate(function () {
      try {
        var cinInfo = gdsn.dom.getMessageInfoForDom($cin)
        ...
        if (cinInfo.receiver !== gdsn.opts.homeDataPoolGln) {
          return cb(new Error('createCinResponse: message must be addressed to home data pool GLN ' + gdsn.opts.homeDataPoolGln))
        }

        var respTemplateFilename = gdsn.opts.templatePath + '/GDSNResponse_template.xml'

        gdsn.dom.getXmlDomForFile(respTemplateFilename, function (err, $response) {
          if (err) return cb(err)

          cinInfo.resId = 'cin_resp_' + Date.now()
          gdsn.dom.populateResponseTemplate($response, cinInfo)
  ...
  gdsn.dom.forwardCinFromOtherDP = function ($cin, cb) {
    setImmediate(function () {
      try {
        ...
        // check that receiver is the home data pool:
        if (cinInfo.receiver !== gdsn.opts.homeDataPoolGln) {
          return cb(new Error('forwardCinFromOtherDP: message must be addressed to home data pool GLN ' + gdsn.opts.homeDataPoolGln))
        }
        // set sender to home data pool
        gdsn.dom.setNodeData($cin, '//*[local-name()="Sender"]/*[local-name()="Identifier"]', gdsn.opts.homeDataPoolGln)
        ...
        if (dataRecipient === gdsn.opts.homeDataPoolGln) {
        ...
        gdsn.dom.setNodeData($cin, '//*[local-name()="message"]/entityIdentification/contentOwner/gln', gdsn.opts.homeDataPoolGln)
