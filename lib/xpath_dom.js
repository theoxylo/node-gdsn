...
module.exports = function addDomSupport(gdsn) {
  ...
  gdsn.dom.processCinFromOtherDp = function (cinInFile, cb) {
    ...
    gdsn.dom.getXmlDomForFile(cinInFile, function(err, cin) {
      ...
      gdsn.dom.createCinResponse(cin, function(err, responseXml) {
        if (err) return cb(err)
        fs.writeFile(responseOutFile, responseXml, function(err, size) {
          if (err) return cb(err)
      ...
      gdsn.dom.forwardCinFromOtherDP(cin, function(err, cinOut) {
        if (err) return cb(err)
        fs.writeFile(forwardOutFile, cinOut, function(err, size) {
          if (err) return cb(err)
  ...
  gdsn.dom.getCustomTradeItemInfo = function (xml, mappings) {
    ...
    var $doc
    try {
      $doc = _xmldom_parser.parseFromString(xml, 'text/xml')
      $doc.normalize()
    }
    catch (err) {
      $doc = {}
      console.log('ignoring parse error: ' + err + ' for xml: ' + xml)
    }
    var mapping
  ...
  gdsn.dom.getTradeItemInfo = function (raw_xml, msg_info) {
    ...
    info.gtin      = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemIdentification/gtin')
    info.provider  = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemInformation/informationProviderOfTradeItem/informationProvider/gln')
    info.tm        = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemInformation/targetMarketInformation/targetMarketCountryCode/countryISOCode')
    info.unit_type = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemUnitDescriptor')
    info.gpc       = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemInformation/classificationCategoryCode/classificationCategoryCode')
    info.brand     = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemInformation/tradeItemDescriptionInformation/brandName')
    info.tm_sub    = gdsn.dom.getNodeData($doc, '/tradeItem/tradeItemInformation/targetMarketInformation/targetMarketSubdivisionCode/countrySubDivisionISOCode')
    if (!info.tm_sub) info.tm_sub = 'na'

    // child items
    info.child_count = gdsn.dom.getNodeData($doc, '/tradeItem/nextLowerLevelTradeItemInformation/quantityOfChildren')
    info.child_gtins = gdsn.dom.getNodeData($doc, '/tradeItem/nextLowerLevelTradeItemInformation/childTradeItem/tradeItemIdentification/gtin', true)
